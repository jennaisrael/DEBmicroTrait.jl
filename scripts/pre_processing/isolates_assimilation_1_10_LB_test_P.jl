...
###########NOTE: make sure the molecular weights are all the same for the same compound (number of decimals, etc.)
df_media_raw            = CSV.read(joinpath(dir, "files/input/LB_medium_with_P.csv"), DataFrame, missingstring="N/A")
############################################################
#EDIT for 1/10 LB multiply column "Concentration" by 0.1
ten_dil(conc) = conc .* 0.1
df_media_dilute=transform!(df_media_raw, :Concentration => ten_dil => :Concentration)

gdf = groupby(df_media_dilute, [:Formula, :Compound, :Ontology, :Molecular_weight]) #need columns named "Formula", "Name"(Name should probably be "Compouund" instead), "Ontology", "Molecular_weight" and "Concentration" (used in the batch model)
unique_metabolites = combine(gdf, :Concentration => sum)
#write this to a CSV
CSV.write(joinpath(dir, "files/output2/1_10_LB_medium_unique_test_P.csv"),unique_metabolites)
df_metabolites          = CSV.read(joinpath(dir, "files/output2/1_10_LB_medium_unique_test_P.csv"), DataFrame, missingstring="N/A")
########################################
...
save("./files/output/isolates_assimilation_1_10_LB_test_P.jld", "rho", ρ_ps, "NSB", N_SB, "KD", K_D, "yEM", y_EM, "yDE", y_DEs, "NC", N_C)